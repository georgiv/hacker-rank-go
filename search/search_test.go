...
func TestWhatFlavors(t *testing.T) {
	var tests = []struct {
		...
		{[]int32{2, 1, 3, 5, 6}, 5, 1, 3},
		{[]int32{1, 4, 5, 3, 2}, 4, 1, 4},
		{[]int32{2, 2, 4, 3}, 4, 1, 2},
	}

	for _, test := range tests {
		if cost1, cost2 := whatFlavors(test.cost, test.money); cost1 != test.expected1 || cost2 != test.expected2 {
			t.Errorf("whatFlavors(%v, %d) = (%d, %d), expected = (%d, %d)",
				test.cost,
				test.money,
				cost1,
				cost2,
...
func TestSwapNodes(t *testing.T) {
	var tests = []struct {
		...
		{[][]int32{{2, 3}, {-1, -1}, {-1, -1}},
			[]int32{1, 1},
			[][]int32{{3, 1, 2}, {2, 1, 3}}},
		{[][]int32{{2, 3}, {-1, 4}, {-1, 5}, {-1, -1}, {-1, -1}},
			[]int32{2},
			[][]int32{{4, 2, 1, 5, 3}}},
		{[][]int32{{2, 3}, {4, -1}, {5, -1}, {6, -1}, {7, 8}, {-1, 9}, {-1, -1}, {10, 11}, {-1, -1}, {-1, -1}, {-1, -1}},
			[]int32{2, 4},
			[][]int32{{2, 9, 6, 4, 1, 3, 7, 5, 11, 8, 10}, {2, 6, 9, 4, 1, 3, 7, 5, 10, 8, 11}}},
	}

	for _, test := range tests {
		if result := swapNodes(test.indexes, test.queries); !matrixDeepEqual(result, test.expected) {
			t.Errorf("swapNodes(%v, %v) = %v, expected = %v", test.indexes, test.queries, nil, test.expected)
		}
	}
}
...
func TestPairs(t *testing.T) {
	...
	for _, test := range tests {
		if result := pairs(test.target, test.ns); result != test.expected {
			t.Errorf("pairs(%d, %v) = %d, expected = %d", test.target, test.ns, result, test.expected)
		}
	}
}
...
func TestTriplets(t *testing.T) {
	var tests = []struct {
		...
		{[]int32{1, 4, 5}, []int32{2, 3, 3}, []int32{1, 2, 3}, 5},
		{[]int32{1, 3, 5, 7}, []int32{5, 7, 9}, []int32{7, 9, 11, 13}, 12},
		{[]int32{1, 4, 5}, []int32{2, 3, 3}, []int32{1, 2, 2}, 4},
	}

	for _, test := range tests {
		if result := triplets(test.a, test.b, test.c); result != test.expected {
			t.Errorf("triplets(%v, %v, %v) = %d, expected = %d", test.a, test.b, test.c, result, test.expected)
		}
	}
}
...
func TestMinimumPasses(t *testing.T) {
	var tests = []struct {
		...
		{1, 1, 499999999999, 1000000000000, 999999999999},
		{7, 1, 456, 398, 57},
		{1, 2, 343, 270, 135},
	...
	for _, test := range tests {
		if result := minimumPasses(test.machines, test.workers, test.price, test.target); result != test.expected {
			t.Errorf("minimumPasses(%d, %d, %d, %d) = %d, expected = %d", test.machines, test.workers, test.price, test.target, result, test.expected)
		}
	}
}
...
func TestMaximumSum(t *testing.T) {
	...
	for _, test := range tests {
		if result := maximumSum(test.a, test.m); result != test.expected {
			t.Errorf("maximumSum(%v, %d) = %d, expected = %d", test.a, test.m, result, test.expected)
		}
	}
}

func matrixDeepEqual(a, b [][]int32) bool {
	if len(a) != len(b) {
		return false
	}

	for i, v := range a {
		if !arrayDeepEqual(v, b[i]) {
			return false
		}
	}
